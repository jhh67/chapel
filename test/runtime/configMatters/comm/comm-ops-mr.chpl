...
config const numTasks = numLocales;
config const printTime = true;
config const printDiags = false;

enum OP {GET,PUT,FAMO,NFAMO,CASAMO,GETAMO,PUTAMO};
...
var A = Block.createArray(1..numTasks*numLocales, padded(atomic int));
var B = Block.createArray(1..numTasks*numLocales, padded(int));
for loc in Locales do on loc {
  coforall tid in 1..numTasks*numLocales {
    A[tid].val.write(0);
    B[tid].val = 0;
  }
}
