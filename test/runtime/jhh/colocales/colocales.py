...
"""
These are utility functions and the test cases (which start with 'test_').
The tests in this class are not run directly, but instead are run from
subclasses that defined different machine topologies. The outer class
...
class TestCases(object):
    class TestCase(unittest.TestCase):
        ...
        """
        Locale in NUMA domain should use cores and threads in that domain and
        the closest NIC.
        """
        def test_03_in_numa(self):
            # Locale in NUMA domain should use closest NIC.
            if self.numas == 1:
                self.skipTest("one NUMA domain per socket")
            ...
            for i in range(0, numas):
                with self.subTest(i=i):
                    output = self.runCmd("./colocales -r %d -n %d -N %s" %
                                     (i, numas, self.nicForNuma[i]),
                                     env=env)
                    cpus = stringify(self.getNumaCores(i))
                    self.assertIn("Physical CPUs: %s\n" % cpus, output)
                    cpus = stringify(self.getNumaThreads(i))
                    self.assertIn("Logical CPUs: %s\n" % cpus, output)
                    self.assertIn("NIC: " + self.nicForNuma[i], output)
...
class M6inTests(TestCases.TestCase):
    ...
    """
    The base test_02_in_socket implementation doesn't work because the NICs
    are not in sockets. Instead, check that whichever NIC the locale in
    socket 0 uses, the locale in socket 1 uses the other one.
    """
    def test_02_in_socket(self):
        # XXX
