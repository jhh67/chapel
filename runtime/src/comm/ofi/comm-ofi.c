...
void init_ofiEp(void) {
  ...
  for (int i = numWorkerTxCtxs; i < tciTabLen; i++) {
    init_ofiEpTxCtx(i, true /*isAMHandler*/, &cqAttr, 
...
size_t handleAmReq(amRequest_t *req) {
  ...
  switch (req->b.op) {
    ...
    default:
      ...
      chpl_comm_ofi_abort_on_error = true;
...
void processRxAmReqCntr(void) {
  ...
  for (int i = 0; i < todo; i++) {
    ...
    char *horizon = MIN(ptr + 2 * sizeof(amRequest_t), (char *) ofi_rxEnd);
    ...
    if (ptr >= horizon) {
      ...
      horizon = MIN(ptr + 2 * sizeof(amRequest_t), (char *) rxEnd);
