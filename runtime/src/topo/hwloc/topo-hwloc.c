...
#ifdef NOTDEF
  for (int d = 0; d < 10; d++) {
    for (hwloc_obj_t obj = hwloc_get_next_obj_inside_cpuset_by_depth(topology,
                                    logAccSet, d, NULL);
         obj != NULL;
         obj = hwloc_get_next_obj_inside_cpuset_by_depth(topology,
                                      logAccSet, d, obj)) {
        _DBG_P("obj depth %d type %s", obj->depth,
...
// hwloc_get_common_ancestor_obj function does not work on objects without
// cpusets such as I/O objects, so first find a non-I/O ancestor of the
// specified objects.
//
static int
      ...
      if (numNics > 1) {
        ...
        while (!finished && (numAssigned < numLocales)) {
          ...
          while((numAvail > 0) && (numAssigned < numLocales)) {
              ...
              _DBG_P("minNic %d minLoc %d", minNic, minLoc);
